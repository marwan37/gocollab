...
`
  ...
  const editorDidMount = (editor: any, monaco: any) => {
    ...
    const url = createUrl(props.langServer.host, props.langServer.port, props.langServer.path);
...
function createUrl(host: string, port: string, path: string) {
  ...
  return normalizeUrl(
    `${protocol}://${host + ':' + port}${window.location.pathname === '/' ? '' : window.location.pathname}${path}`
  );
}
